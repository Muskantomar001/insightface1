...
Original file is located at
    https://colab.research.google.com/drive/1epBjpFpBA0Y76d-hBLsbEmcIo4OZl-Qz
"""
...
def extract(lfw_dataset):
    with ZipFile(lfw_dataset, 'r') as zip_ref:
        zip_ref.extractall(extracted_datasetpath)




def process():
    subjects = [d for d in os.listdir('/content/lfw_dataset.zip') if os.path.isdir(os.path.join('/content/lfw_dataset.zip', d))]
    assert (len(subjects) == 5749), "Number of subjects is: {}!".format(len(subjects))

    file_names = []
    for i in tqdm(range(len(subjects))):
        sub = subjects[i]
        folder = os.path.join('/content/lfw_dataset.zip', sub)
...
def lfw_test(model):
    filename = '/content/lfw_dataset.zip'
    if not os.path.isdir('lfw_dataset'):
